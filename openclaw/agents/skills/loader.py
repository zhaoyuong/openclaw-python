...
def load_skills_from_dir(directory: Path | str, source: str = "workspace") -> list[Skill]:
    """
...
def load_skill_entries_from_dir(
    directory: Path | str, source: str = "workspace"
) -> list[SkillEntry]:
    """
...
def load_skill_from_file(file_path: Path, source: str = "workspace") -> Skill | None:
    """
    ...
    return Skill(name=name.strip(), description=description.strip(), location=str(file_path))


def load_skill_entry_from_file(file_path: Path, source: str = "workspace") -> SkillEntry | None:
    """
    ...
    return SkillEntry(
        skill=skill, frontmatter=frontmatter, metadata=metadata, invocation=invocation
    )
