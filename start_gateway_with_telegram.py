...
async def main():
    ...
    try:
        ...
        from openclaw.agents.system_prompt_params import (
            build_system_prompt_params,
        )
        ...
        logger.info("   📋 将使用注册的 19 个工具（包括 web_search）")
        ...
        tool_registry = ToolRegistry(
            session_manager=session_manager, auto_register=True  # 自动注册默认工具
        )
        ...
        try:
            ...
            logger.info(
                f"✅ Skills 加载成功，{len(all_skills)} 个总计，{len(eligible_skills)} 个合格"
            )
            ...
            skills_for_prompt = [
                {
                    ...
                    "location": (
                        str(Path(skill.content).parent / "SKILL.md")
                        if hasattr(skill, "file_path")
                        else ""
                    ),
        ...
        params = build_system_prompt_params(
            ...
            runtime={"agent_id": "main", "model": model, "channel": "telegram"},
        )
        ...
        logger.info("  - Telegram Bot: @whatisnewzhaobot")
        logger.info("  - DM Policy: open (允许所有人)")
